...
const CHAT_SESSION_STORAGE_KEY = 'vyuha_chat_session_id_v2';
const LEGACY_CHAT_SESSION_STORAGE_KEY = 'vyuha_chat_session_id';

const toMessageTimestamp = (createdAt) => {
...
const ChatAssistant = () => {
      ...
      try {
        ...
        if (existingSessionId) {
          ...
          const loaded = (historyRes.messages || []).map((msg, idx) => ({
            ...
            content: msg.content,
            timestamp: toMessageTimestamp(msg.created_at),
          }));
          if (loaded.length > 0) {
            setMessages(loaded);
          }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const errorMessage = normalizeApiError(
        error,
        'I could not process that request. Try again with a teacher name, day, time, or replacement teacher.'
      );
      setMessages(prev => [...prev, {
        id: prev.length + 1,
        type: 'bot',

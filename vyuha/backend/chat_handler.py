...
ROOM_AVAILABILITY_PATTERNS = [
    r"\b(?:available|empty|free|vacant|unused|open)\s+(?:class\s*)?rooms?\b",
    r"\b(?:class\s*)?rooms?\s+(?:available|empty|free|vacant|unused|open)\b",
    r"\b(?:any|which|list|show|get|find)\b.*\b(?:rooms?|labs?)\b.*\b(?:available|empty|free|vacant|unused|open)\b",
    r"\b(?:available|empty|free|vacant|unused|open)\b.*\b(?:rooms?|labs?)\b",
]
...
def _is_room_availability_query(message: str) -> bool:
    lowered = (message or "").lower().strip()
    if not lowered:
        return False
    return any(re.search(pattern, lowered, re.IGNORECASE) for pattern in ROOM_AVAILABILITY_PATTERNS)


def _default_schedule_time() -> str:
...
async def execute_query_timetable(
    ...
    if empty_only and not time_slot:
        time_slot = _default_schedule_time()
    ...
    try:
        ...
        res = query.execute()
        if empty_only or (not fac_id and not room_id and not sub_id and day and formatted_time):
            if not day or not formatted_time:
                return "Tell me the day and time to check empty rooms."
            all_rooms_res = supabase.table("rooms").select("id,room_name,room_code").eq("college_id", college_id).execute()
            if not all_rooms_res.data:
                return "No rooms exist in the database for this college."

            all_rooms = {r["id"]: (r.get("room_name") or r.get("room_code") or f"Room {r['id']}") for r in all_rooms_res.data}

            today_slots_res = supabase.table("timetable_slots").select("room_id, start_time, end_time").eq("college_id", college_id).eq("day", day).execute()
            today_slots = today_slots_res.data or []

            occupied_ids = set()
            room_next_class = {}
            check_time_obj = datetime.datetime.strptime(formatted_time[:5], "%H:%M").time()

            for slot in today_slots:
                r_id = slot.get("room_id")
                if not r_id:
                    continue

                s_time_str = slot.get("start_time", "")[:5]
                e_time_str = slot.get("end_time", "")[:5]
                if not s_time_str or not e_time_str:
                    continue

                s_time = datetime.datetime.strptime(s_time_str, "%H:%M").time()
                e_time = datetime.datetime.strptime(e_time_str, "%H:%M").time()

                if s_time <= check_time_obj < e_time:
                    occupied_ids.add(r_id)
                elif s_time >= check_time_obj:
                    if r_id not in room_next_class or s_time < room_next_class[r_id]:
                        room_next_class[r_id] = s_time

            empty_rooms = []
            for rid, name in all_rooms.items():
                if rid not in occupied_ids:
                    until_time = room_next_class.get(rid)
                    until_str = until_time.strftime("%H:%M") if until_time else "rest of the day"
                    empty_rooms.append((name, until_str))

            if not empty_rooms:
                return f"All rooms are occupied on **{day}** at **{formatted_time[:5]}**."

            res_str = f"🏢 **Available Rooms on {day} at {formatted_time[:5]}:**\n"
            for room_name, until_str in empty_rooms[:15]:
                res_str += f"- **{room_name}** (Free until: {until_str})\n"
            if len(empty_rooms) > 15:
                res_str += f"*(And {len(empty_rooms) - 15} more...)*\n"
...
def _extract_query_entities(message: str, college_id: str = None):
    ...
    empty_only = _is_room_availability_query(normalized)
    ...
    time_match = re.search(r"\b(\d{1,2}:\d{2})\s*(am|pm)?\b", normalized, re.IGNORECASE)
    time_slot = None
    if time_match:
        hour, minute = time_match.group(1).split(":", 1)
        hour_int = int(hour)
        meridiem = (time_match.group(2) or "").lower()
        if meridiem == "pm" and hour_int < 12:
            hour_int += 12
        elif meridiem == "am" and hour_int == 12:
            hour_int = 0
        elif not meridiem and empty_only and 1 <= hour_int <= 6:
            hour_int += 12
        time_slot = f"{hour_int:02d}:{minute}"
    elif empty_only:
        time_slot = _default_schedule_time()

    if empty_only and not day:
        day = datetime.datetime.now().strftime("%a")
    ...
    room_match = None
    if not empty_only:
        room_match = re.search(r"\b(?:room|lab)\s+([A-Za-z0-9][A-Za-z0-9 .'-]{0,49})", normalized, re.IGNORECASE)
    if room_match:
...
def _classify_chat_intent(
    ...
    lowered = (message or "").lower().strip()
    if _is_room_availability_query(message):
        return "schedule"
...
def _has_substitution_context(message: str, history: Optional[List[dict]], context: Optional[dict], college_id: str = None) -> bool:
    entities = _extract_query_entities(message, college_id)
    is_explicit_schedule = any(re.search(pattern, message.lower(), re.IGNORECASE) for pattern in SCHEDULE_PATTERNS)

    if is_explicit_schedule and not entities.get("faculty_name"):
        return False

    if not is_explicit_schedule and entities.get("faculty_name"):
        return True
...
async def chat_interaction(
    ...
    try:
        ...
        if (
            _is_followup_availability_question(request.message)
            and not _is_room_availability_query(request.message)
            and recent_substitutes
        ):
        ...
        # Schedule and room lookups are deterministic. Letting the model choose between
        # timetable and substitution tools caused room/schedule questions to be misrouted.
        if intent == "schedule":
